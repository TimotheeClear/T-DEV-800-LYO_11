<template>
  <ion-content >
    <div class="padding">
      ...
      <div class="logo">
        <ion-icon :icon="personCircleOutline" color="light"></ion-icon>
      </div>
      
      <ion-item >
        <ion-label color="light">Email</ion-label>
        <ion-input color="light" placeholder="Enter your email" v-model="email"></ion-input>
      </ion-item>
  
      <ion-item>
        <ion-label color="light">Mot de passe</ion-label>
        <ion-input color="light" type="password" placeholder="Enter your password" v-model="password"></ion-input>
      </ion-item>
  
      <ion-item lines="none">
        <ion-button color="light" slot="end" fill="clear"></ion-button>
      </ion-item>
  
      <ion-button color="google" expand="block" shape="round" @click="login(email, password)">Connexion</ion-button>
      <br>
...
import {IonIcon, IonContent, IonItem, IonLabel, IonInput} from '@ionic/vue';
import {personCircleOutline } from 'ionicons/icons';

import { userAuthentification } from '@/composables/userAuthentification';
import { ref } from 'vue';
const { login } = userAuthentification();
...
ion-content {
  --ion-background-color: linear-gradient(#485563, #29323c);
}
ion-item {
